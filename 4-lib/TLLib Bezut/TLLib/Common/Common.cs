...
namespace TLLib
{
    ...
    public class Common
    {
        ...
        public void RecursiveFillTree1(DataTable dtParent, int parentID, string parentColumnName, string displayColumnName, string valueColumnName, int increaseLevelCount, string IsShowOnMenu, string IsShowOnHomePage, string IsTitle)
        {
            if (dt.Columns.Count == 0)
            {
                foreach (DataColumn dc in dtParent.Columns)
            ...
            dv.RowFilter = string.Format(parentColumnName + " = {0} AND IsAvailable = 1 " +
                (string.IsNullOrEmpty(IsShowOnMenu) ? "" : " AND IsShowOnMenu = " + IsShowOnMenu) +
                (string.IsNullOrEmpty(IsShowOnHomePage) ? "" : " AND IsShowOnHomePage = " + IsShowOnHomePage) +
                (string.IsNullOrEmpty(IsTitle) ? "" : " AND IsTitle = " + IsTitle)
            , parentID);
            ...
            if (dv.Count > 0 && (level <= increaseLevelCount - 1 || increaseLevelCount == -1))
            {
                foreach (DataRowView drv in dv)
                {
                    ...
                    obj[columnLength - 1] = level;
                    dt.Rows.Add(obj);
                    RecursiveFillTree1(dtParent, int.Parse(drv[valueColumnName].ToString()), parentColumnName, displayColumnName, valueColumnName, increaseLevelCount, IsShowOnMenu, IsShowOnHomePage, IsTitle);
                }
            }
