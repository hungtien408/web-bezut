...
namespace TLLib
{
    public class Comment
    {
        ...
        public DataTable CommentSameSelectAll(
            string CommentID
        )
        {
            try
            {
                var dt = new DataTable();
                var scon = new SqlConnection(connectionString);
                var cmd = new SqlCommand("usp_SameComment_SelectAll", scon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@CommentID", string.IsNullOrEmpty(CommentID) ? dbNULL : (object)CommentID);
                SqlParameter errorCodeParam = new SqlParameter("@ErrorCode", null);
                errorCodeParam.Size = 4;
                ...
                if (errorCodeParam.Value.ToString() != "0")
                    throw new Exception("Stored Procedure 'usp_SameComment_SelectAll' reported the ErrorCode : " + errorCodeParam.Value.ToString());
